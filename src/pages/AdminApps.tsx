...
function AppEditorModal({ app, platform, isNew, onSave, onClose }: AppEditorModalProps) {
  ...
  const [activeTab, setActiveTab] = useState<
    'basic' | 'installation' | 'subscription' | 'connect' | 'additional'
  >('basic');
      ...
      <div className="space-y-2">
        ...
        {(['en', 'ru'] as const).map((lang) => (
          <div key={lang} className="flex items-center gap-2">
            <span className="w-8 text-xs uppercase text-dark-500">{lang}</span>
            <textarea
              value={value[lang] || ''}
    ...
    return (
      <div className="space-y-3">
        ...
        {buttons.map((button, index) => (
          <div
            key={button.id || `fallback-${index}`}
            className="space-y-2 rounded-lg bg-dark-800/50 p-3"
          >
            <div className="flex items-center justify-between">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl bg-dark-900">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-dark-700 p-4">
          <h2 className="text-lg font-semibold text-dark-100">
            {isNew ? t('admin.apps.createApp') : t('admin.apps.editApp')} -{' '}
        ...
        <div className="flex overflow-x-auto border-b border-dark-700">
          {(['basic', 'installation', 'subscription', 'connect', 'additional'] as const).map(
            (tab) => (
              <button
                key={tab}
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <label className="flex cursor-pointer items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      updateField('isNeedBase64Encoding', e.target.checked || undefined)
                    }
                    className="h-4 w-4 accent-accent-500"
                  />
          ...
          {activeTab === 'installation' && (
            <>
              {renderLocalizedTextInputs(
                'installationStep',
                'description',
                t('admin.apps.installDescription'),
              )}
          ...
          {activeTab === 'subscription' && (
            <>
              {renderLocalizedTextInputs(
                'addSubscriptionStep',
                'description',
                t('admin.apps.subscriptionDescription'),
              )}
            </>
          )}
          ...
          {activeTab === 'connect' && (
            <>
              {renderLocalizedTextInputs(
                'connectAndUseStep',
                'description',
                t('admin.apps.connectDescription'),
              )}
            </>
          )}
            ...
            <>
              <div className="space-y-4">
                <h3 className="text-sm font-semibold text-dark-200">
                  {t('admin.apps.beforeSubscription')}
                </h3>
                {renderLocalizedTextInputs(
                  'additionalBeforeAddSubscriptionStep',
                  'title',
                  t('admin.apps.stepTitle'),
                )}
                {renderLocalizedTextInputs(
                  'additionalBeforeAddSubscriptionStep',
                  'description',
                  t('admin.apps.stepDescription'),
                )}
                {renderButtonsEditor('additionalBeforeAddSubscriptionStep')}
              </div>
              <div className="space-y-4 border-t border-dark-700 pt-4">
                <h3 className="text-sm font-semibold text-dark-200">
                  {t('admin.apps.afterSubscription')}
                </h3>
                {renderLocalizedTextInputs(
                  'additionalAfterAddSubscriptionStep',
                  'title',
    ...
    <div className="space-y-6">
      ...
      {useRemnaWave && (
        <div className="flex items-center gap-2 rounded-lg border border-accent-500/30 bg-accent-500/10 p-3 text-sm text-accent-200">
          ...
          <span>
            {t(
              'admin.apps.remnaWaveMode',
              'Showing apps from RemnaWave panel. Changes here are read-only.',
            )}
          </span>
        </div>
      )}
          ...
          {apps.map((app, index) => (
            <div key={app.id} className="card flex items-center gap-4 p-4">
              ...
              {!useRemnaWave && (
                <div className="flex flex-col gap-1">
                  <button
                    ...
                    disabled={index === 0}
                    className="p-1 text-dark-400 hover:text-dark-200 disabled:cursor-not-allowed disabled:opacity-30"
                  >
                    <ChevronUpIcon />
                  </button>
                  <button
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate font-mono text-xs text-dark-500">{app.id}</div>
                <div className="mt-1 text-xs text-dark-400">{app.urlScheme}</div>
              </div>
              ...
              {!useRemnaWave && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="rounded p-2 text-dark-400 hover:bg-dark-700 hover:text-dark-200"
                  ...
                  <button
                    ...
                    className="rounded p-2 text-error-400 hover:bg-error-500/10 hover:text-error-300"
      ...
      {copyTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-xl bg-dark-900 p-6">
            <h3 className="mb-4 text-lg font-semibold text-dark-100">{t('admin.apps.copyTo')}</h3>
            <div className="grid grid-cols-2 gap-2">
              {PLATFORMS.filter((p) => p !== copyTarget.platform).map((platform) => (
                <button
                  ...
                  className="rounded-lg bg-dark-800 px-4 py-2 text-sm text-dark-200 hover:bg-dark-700"
                >
                  {PLATFORM_LABELS[platform]}
                </button>
              ))}
            </div>
            <button onClick={() => setCopyTarget(null)} className="btn-secondary mt-4 w-full">
              {t('common.cancel')}
            </button>
          </div>
        </div>
      ...
      {showImportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="flex max-h-[80vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl bg-dark-900 p-6">
            <h3 className="mb-4 text-lg font-semibold text-dark-100">
              {t('admin.apps.importPreview', 'Import Preview')}
            </h3>

            {importError && (
              <div className="mb-4 rounded-lg border border-error-500/30 bg-error-500/10 p-3 text-error-400">
                {importError}
              </div>
            )}
            ...
            {importPreview && (
              <div className="mb-4 flex-1 space-y-4 overflow-y-auto">
                {Object.entries(importPreview.platformApps).map(
                  ([platform, platformApps]) =>
                    platformApps.length > 0 && (
                      <div key={platform} className="rounded-lg bg-dark-800/50 p-4">
